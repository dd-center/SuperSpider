        ...
        case 'GUARD_BUY':
          {
            ...
            try {
              ...
              try {
                const avatard = JSON.parse(
                  await rp(
                    'https://api.bilibili.com/x/space/acc/info?mid=' + data.mid
                  )
                )
                if (Number(avatard.code) == 0) avatar = avatard.data.face
                else
                ...
                console.log(
                  'ERR when fetching face of uid ' + data.mid + ' by catch'
                )
                console.log(e)
              }
              if (
                ...
                if (preList[rid].id) {
                  await predb.updateOne(
                    { _id: ObjectId(preList[rid].id) },
                    {
                      $set: {
                        num: Number(
                          Number(preList[rid].num) + Number(data.num)
                        ),
                        price: Number(
                          Number(preList[rid].price) +
